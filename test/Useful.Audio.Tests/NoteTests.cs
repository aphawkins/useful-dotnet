...
namespace Useful.Audio.Tests
{
    ...
    {
        [Fact]
        public void OctaveTooLow() => Assert.Throws<ArgumentOutOfRangeException>(() => new Note(-2, NoteStep.C, NoteDuration.Zero, new(PartialNote.Whole)));

        [Fact]
        public void OctaveTooHigh() => Assert.Throws<ArgumentOutOfRangeException>(() => new Note(10, NoteStep.C, NoteDuration.Zero, new(PartialNote.Whole)));

        [Fact]
        public void DurationIsNegative() => Assert.Throws<ArgumentOutOfRangeException>(() => new Note(4, NoteStep.C, NoteDuration.Zero, new((PartialNote)(-1))));
