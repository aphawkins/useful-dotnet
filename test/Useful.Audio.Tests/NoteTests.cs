...
namespace Useful.Audio.Tests
{
    ...
    {
        [Fact]
        public void OctaveTooLow() => Assert.Throws<ArgumentOutOfRangeException>(() => new Note(-2, NoteStep.C, TimeSpan.FromSeconds(1)));

        [Fact]
        public void OctaveTooHigh() => Assert.Throws<ArgumentOutOfRangeException>(() => new Note(10, NoteStep.C, TimeSpan.FromSeconds(1)));
